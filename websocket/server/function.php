...
//获取用户数据
function get_current_user_info($sessId){
	global $db;
	$sql = "select * from sp_session where session_id='{$sessId}' limit 1";
	$res = $db->query($sql);
	if(empty($res)){
		return false;
	}
	$data = $res[0]['session_data'];
	$data = unserialize($data);
	return $data['user'];
}
//通过name(user_realname)来获取用户uid
function get_user_id($name){
	global $db;
	$sql = "select id from sp_member where user_realname='{$name}'";
	$data = $db->query($sql);
	if(empty($data)){
		return false;
	}
